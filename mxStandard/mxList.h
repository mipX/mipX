...
namespace mxListFunctions
{
    ...
    /// Check if the list has the input double value (for list of doubles) with the given precision.
    template <typename T>
    int HasDoubleValue(mxList<T> &list, double &value, double precision);
...
template<class T>
int mxListFunctions::HasDoubleValue(mxList<T> &list, double &value, double precision)
{
    //if(list.IsEmpty()) return 0;
    mxListIterator<T> it;
    for(it.SetToBegin(list); it.IsValid(); it.MoveToNext())
    {
        double d = (*(it.GetElementAddress()));
        if( value < d+precision && value > d-precision) return 1;
    }
    return 0;
}
