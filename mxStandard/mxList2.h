...
private:
    ...
    /// Pointer to the assigned list.
    mxList<T> *m_list;
    
    /// Iterator is a standard iterator.
...
{
    m_list = &list;
    m_iterator = list.m_std_list->begin();
}
...
{
    m_list = &list;
    m_iterator = list.m_std_list->rbegin();
}
...
{
	if(m_iterator == m_list->end()) return 0;
    if(m_iterator == m_list->rend()) return 0;
    return 1;
}
...
//template<class T>
//using mxListNode = typename std::list<T>::iterator;
...
public:
    ...
    /// Takes over possesion of input list. If this object is released, the list will also be released.
    int AttachStdList(std::list<T> *list_to_attach);
...
template <class T>
int mxList<T>::AttachStdList(std::list<T> *list_to_attach)
{
    if(!list_to_attach) return 0;
    
    delete m_std_list;
    
    m_std_list = list_to_attach;
...
void mxList<T>::Reset()
{
    while(!m_std_list->empty())
    {
        m_std_list->pop_front();
    }
}
